...
@implementation AppreciatePoemViewModel
...
- (void)getDataFromNetCompleteHandle:(CompletionHandle)completionHandle {
    self.dataTask = [MetreNetManager getApprecicatePoemWithWithPageIndex:self.pageIndex CompletionHandle:^(PoemCommonSenseModel *model, NSError *error) {
        if (!error) {
            if (self.pageIndex == 0) {
